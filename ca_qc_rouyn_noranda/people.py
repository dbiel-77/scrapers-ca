...
class RouynNorandaPersonScraper(CanadianScraper):
    def scrape(self):
        ...
        mayor_email_a = mairie_h2[0].xpath('following-sibling::a[starts-with(@href,"mailto:")][1]')
        mayor_email = mayor_email_a[0].get("href").replace("mailto:", "") if mayor_email_a else None
        p = Person(primary_org="legislature", name=mayor_name, district="Rouyn-Noranda", role="Mayor")
        ...
        district_h3s = [h for h in all_h3 if re.match(r"District\s+\d+", h.text_content().strip())]
