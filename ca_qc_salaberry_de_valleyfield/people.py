...
class SalaberryDeValleyfieldPersonScraper(CanadianScraper):
    def scrape(self):
        ...
        profile_links = list(
            dict.fromkeys(page.xpath('//h3/a[contains(@href,"/conseil-municipal/")]/@href'))
        )
        ...
        for href in profile_links:
            ...
            role_els = body.xpath(
                './/*[contains(text(),"Maire") or contains(text(),"Conseiller")]'
            )
            ...
            if "Maire" in role_text and "Conseiller" not in role_text:
                ...
                m = re.search(
                    r"district[- ]?(\d+)", url + " " + body.text_content(), re.I
                )
                if m:
                    num = int(m.group(1))
                else:
            ...
            email = (
                email_el[0].get("href").replace("mailto:", "") if email_el else None
            )
            phone_link = ppage.xpath('.//a[starts-with(@href,"tel:")]')
            phone = phone_link[0].text_content().strip() if phone_link else None
            image = ppage.xpath(
                '//img[contains(@src,"/media/") or contains(@src,"conseil")]/@src'
            )
